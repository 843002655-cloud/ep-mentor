    ...
    <>
      ...
      <footer className="border-t border-[#E8ECF0] bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
            ...
            <div className="col-span-2 md:col-span-1">
              <Link href="/" className="flex items-center gap-2 mb-3">
                ...
                <span className="text-lg font-bold text-[#1A2332] font-serif">
                  EP <span className="text-[#1B4F8A]">Mentor</span>
                </span>
              </Link>
              <p className="text-sm text-[#8FA0B4] leading-relaxed">
                心脏电生理 AI 教学平台
                <br />
            ...
            {Object.entries(footerLinks).map(([title, links]) => (
              <div key={title}>
                <h4 className="text-sm font-semibold text-[#1A2332] mb-3">{title}</h4>
                <ul className="space-y-2">
                  {links.map((l) => (
                    <li key={l.label}>
                      <Link
                        href={l.href}
                        className="text-sm text-[#6B7F96] hover:text-[#1B4F8A] transition-colors"
                      >
                        {l.label}
                      </Link>
                    </li>
          ...
          {/* Bottom bar */}
          <div className="border-t border-[#E8ECF0] pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[#8FA0B4]">
            <p>
