...
export default function AdminGeneratePage() {
  ...
  // Book (PDF case book → one-click import)
  ...
  interface BookCase {
    ...
    status: "pending"|"generating"|"done"|"failed"|"saved";
  ...
  // ── Book (PDF case book → one-click import) ──────────────────
  const doBookImport = async () => {
    if(!bookFile)return;
    setBookImporting(true); setBookErr(""); setBookCases([]);

    try {
      // Phase 1: Extract text
      setBookStep("extracting");
      ...
      // Phase 2: Split into individual cases
      setBookStep("splitting");
      const splitRes=await fetch("/api/split-pdf-cases",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text})});
      const splitData=await splitRes.json();
      if(!splitRes.ok) throw new Error(splitData.error||"拆分失败");

      const cases: BookCase[] = (splitData.cases as BookCase[]).map((c) => ({...c,status:"pending" as const,generated:null,error:""}));
      setBookCases(cases);

      // Phase 3: Generate each case
      for(let i=0;i<cases.length;i++){
        setBookStep(`generating ${i+1}/${cases.length}`);
        setBookCases(prev => prev.map(c => c.index===cases[i].index?{...c,status:"generating" as const,error:""}:c));
        try {
          const genRes=await fetch("/api/generate-book-case",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:cases[i].text_chunk,case_index:cases[i].index,case_title:cases[i].title})});
          const genData=await genRes.json();
          if(!genRes.ok) throw new Error(genData.error||"生成失败");
          setBookCases(prev => prev.map(c => c.index===cases[i].index?{...c,status:"done" as const,generated:genData.case,error:""}:c));
        } catch(e) {
          setBookCases(prev => prev.map(c => c.index===cases[i].index?{...c,status:"failed" as const,error:(e as Error).message}:c));
        }
      }

      // Phase 4: Save all generated cases
      setBookStep("saving");
      let saved=0;
      setBookCases(prev => {
        for(const c of prev){ if(c.status==="done"&&c.generated){ saved++;
          caseService.createCase(flattenCase(c.generated as Record<string,unknown>, {source_book: c.title}) as never).catch(()=>{});
        }}
        return prev.map(c => c.status==="done"?{...c,status:"saved" as const}:c);
      });

      setBookStep("done");
      setBookErr(`✅ 完成！共 ${cases.length} 个病例，成功生成并保存 ${saved} 个`);
    } catch(e) {
      setBookStep(""); setBookErr((e as Error).message);
    } finally { setBookImporting(false); }
  };
    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        {tab==="book"&&<div className="card mb-6">
          ...
          />
          <button onClick={doBookImport} disabled={bookImporting||!bookFile} className="btn-primary w-full text-sm mb-3">
            {bookImporting
              ? (bookStep.startsWith("generating") ? `⏳ ${bookStep}`
                : bookStep==="extracting" ? "📖 提取文字中..."
                : bookStep==="splitting" ? "🔍 拆分病例中..."
                : bookStep==="saving" ? "💾 保存中..."
                : "⏳ 处理中...")
              : `🚀 一键导入病例书${bookFile ? `（${formatFileSize(bookFile.size)}）` : ""}`}
          </button>
          ...
          {bookCases.length>0&&<div className="space-y-1.5 max-h-[50vh] overflow-y-auto mt-3">
              {bookCases.map(bc=>{
                const statusColors: Record<string,string> = {
                  pending: "bg-[#E8ECF0] dark:bg-slate-700 text-[#6B7F96] dark:text-slate-400",
                  generating: "bg-[#EBF2FA] dark:bg-blue-900/20 text-[#1B4F8A] dark:text-blue-400 animate-pulse",
                ...
                const statusLabels: Record<string,string> = {
                  pending:"待生成", generating:"生成中", done:"已生成", failed:"失败", saved:"已保存",
                };
                return (
                  <div key={bc.index} className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[#DDE5EE] dark:border-slate-700 bg-white dark:bg-slate-800 text-sm">
                    <span className="font-bold text-[#1A2332] dark:text-slate-100 w-6">#{bc.index}</span>
                    <span className="flex-1 truncate text-[#3D5166] dark:text-slate-300">{bc.title.slice(0,60)}</span>
                    ...
                    {bc.error&&<span className="text-xs text-[#9B2C2C] dark:text-red-400" title={bc.error}>⚠️</span>}
                  </div>
                );
              })}
          </div>}
