...
const FilterBtn = ({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) => (
  <button onClick={onClick} className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-colors border whitespace-nowrap shrink-0 ${active ? "bg-[#1B4F8A] text-white border-[#1B4F8A]" : "bg-white text-[#4B6080] border-[#C5D3E0] hover:border-[#1B4F8A] hover:text-[#1B4F8A]"}`}>{active && "✓ "}{children}</button>
);
  ...
  return (
    <AppLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        {!loggedIn && (
          <div className="mb-6 sm:mb-8 px-4 py-3 rounded-xl border border-[#1B4F8A]/20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2" style={{ background: "#EBF2FA" }}>
        ...
        <div className="space-y-3 mb-6 sm:mb-8">
          ...
          <div className="relative max-w-sm">
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-white border border-[#C5D3E0] rounded-lg text-sm text-[#1A2332] placeholder-[#8FA0B4] focus:outline-none focus:border-[#1B4F8A] transition-colors"
            />
          ...
          <div className="filter-bar sm:flex-wrap sm:overflow-visible items-center">
            ...
            <div className="w-px bg-[#E8ECF0] mx-1 sm:mx-2 h-6 shrink-0 hidden sm:block" />
            <span className="text-xs text-[#8FA0B4] mr-1 shrink-0 flex items-center">标测</span>
        ...
        : filtered.length===0 ? <div className="text-center py-20"><p className="text-[#6B7F96]">{keyword ? "未找到匹配的病例，试试其他关键词" : "暂无匹配的病例"}</p></div>
        : <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((c) => (
              ...
              >
                ...
                <div className="bg-[#F5F8FC] rounded-lg mb-4 h-28 flex items-center justify-center text-3xl select-none shrink-0">
                  ⚡
                </div>
                ...
                <div className="flex-1">
                  ...
                  <h3 className="text-base sm:text-lg font-semibold text-[#1A2332] mb-2 font-serif group-hover:text-[#1B4F8A] transition-colors line-clamp-2">
                    {c.title}
                  </h3>
