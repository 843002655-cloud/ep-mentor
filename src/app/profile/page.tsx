      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="card mb-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-[#1A2332] font-serif">{user?.email || "用户"}</h1>
            <div className="flex flex-wrap items-center gap-2 mt-1">
              {role && <span className="text-xs px-2 py-0.5 rounded-full bg-[#EBF2FA] text-[#1B4F8A]">{roleLabels[role] || role}</span>}
              {interests.map((k) => (
                <span key={k} className="text-xs px-2 py-0.5 rounded-full bg-[#F5F8FC] text-[#6B7F96]">{subspecialtyLabels[k] || k}</span>
              ))}
            </div>
          </div>
          <Link href={ROUTES.AUTH} className="text-sm text-[#6B7F96] hover:text-[#1B4F8A] transition-colors">编辑资料</Link>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { v: stats.completedCount, l: "已完成病例", c: "text-[#1B4F8A]", icon: "📚" },
            { v: stats.totalCases, l: "总病例数", c: "text-[#4C3D9E]", icon: "📋" },
            { v: stats.todayCount, l: "今日对话", c: "text-[#0F6E56]", icon: "💬" },
            { v: `${stats.completionRate}%`, l: "完成率", c: "text-[#854F0B]", icon: "📊" },
          ].map((s, i) => (
            <div key={i} className="card text-center">
              <span className="text-2xl">{s.icon}</span>
              <div className={`text-2xl font-bold mt-1 ${s.c}`}>{s.v}</div>
              <div className="text-xs text-[#6B7F96] mt-1">{s.l}</div>
            </div>
          ))}
        </div>

        {/* Badges */}
        <h2 className="text-xl font-semibold text-[#1A2332] mb-4 font-serif">🏅 学习徽章</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-8">
          {badges.map((b) => (
            <div key={b.name} className={`card text-center p-4 ${!b.unlocked && "opacity-40"}`}>
              <div className="text-2xl mb-1">{b.icon}</div>
              <div className="text-xs font-medium text-[#1A2332]">{b.name}</div>
              <div className="text-xs text-[#8FA0B4] mt-0.5">{b.unlocked ? "已解锁" : "未解锁"}</div>
            </div>
          ))}
        </div>
        ...
        {progress.length === 0 ? (
          <div className="card text-center text-[#6B7F96]">还没有学习记录，去<a href={ROUTES.CASES} className="text-[#1B4F8A] mx-1">病例库</a>开始学习吧！</div>
        ) : (
          <div className="space-y-3">
            {progress.slice(0, 10).map((p, i) => (
              <div key={i} className="card flex items-center justify-between py-4">
                <div className="flex items-center gap-3">
                  <span className="textlsm text-[#8FA0B4]">{p.cases?.category || "—"}</span>
                  <span className="text-[#1A2332]">{p.cases?.title || "未知"}</span>
                </div>
                <span className="text-xs text-[#8FA0B4]">{new Date(p.completed_at).toLocaleDateString("zh-CN")}</span>
              </div>
            ))}
          </div>
        )}
